  ...
  render() {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-white px-6 text-center">
        ...
        {this.state.error && (
          <p className="text-xs text-gray-400 mb-4 font-mono break-all max-w-xs">
            {this.state.error.message}
          </p>
        )}
        <button
          onClick={this.handleReload}
          className="px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 text-white text-sm font-semibold rounded-xl hover:from-emerald-700 hover:to-teal-700 transition-all"
        >
          Try Again
        </button>
      </div>
