...
class SheetMindAgent:
    ...
    def run(
        ...
        try:
            ...
            return {
                "response": f"I encountered an error while processing your request. Please try rephrasing or simplifying your question.",
                "actions": [],
                "reasoning": [],
                "rows_used": [],
                "used_rag": used_rag,
...
def _cleanup_old_agents():
    ...
    old_sessions = [
        ...
        if now - agent.created_at > AGENT_TIMEOUT_SECONDS
    ]
...
def get_agent(session_id: str = "default") -> SheetMindAgent:
    ...
    # Cleanup old agents if we have too many
    if len(_agents) >= MAX_CACHED_AGENTS:
        _cleanup_old_agents()

    # Still too many? Remove oldest
    if len(_agents) >= MAX_CACHED_AGENTS:
        oldest_sid = min(_agents.keys(), key=lambda s: _agents[s].created_at)
        del _agents[oldest_sid]
        logger.warning(f"Evicted oldest agent {oldest_sid} due to cache limit")
