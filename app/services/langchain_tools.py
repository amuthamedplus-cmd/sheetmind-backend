...
# ---------------------------------------------------------------------------
# Global State (set before each agent run)
# ---------------------------------------------------------------------------

# Current sheet context (set by agent before running)
_current_sheet_context: Dict = {}

# Pending actions to execute in frontend
...
def set_sheet_context(context: Dict) -> None:
    ...
    _current_sheet_context = context or {}


def get_sheet_context() -> Dict:
    """Get the current sheet context."""
    return _current_sheet_context
...
def _queue_action(action: Dict) -> str:
    ...
    if action.get("action") == "createSheet":
        for existing in _pending_actions:
...
def get_data_range() -> str:
    ...
    """
    if not _current_sheet_context:
        return '{"error": "No sheet data available"}'

    info = {
        "sheetName": _current_sheet_context.get("sheetName", "Sheet1"),
        "dataRange": _current_sheet_context.get("dataRange", ""),
    }

    cells = _current_sheet_context.get("cells", {})
    if cells:
        rows = set()
        cols = set()
...
def lookup_formula(intent: str) -> str:
    ...
    metadata = _current_sheet_context.get("metadata", {})
...
def verify_actions() -> dict:
    ...
    for i, action in enumerate(_pending_actions):
        ...
        if action_type == "createChart":
            ...
            if data_sheet not in created_sheets and data_sheet != _current_sheet_context.get("sheetName"):
    ...
    return {
        "total_actions": len(_pending_actions),
